...
- **Docs editor** (`renderDocs`): a Tiptap editor mounted by `bindRichEditor` — the mount element's HTML at bind time is the initial content, every update hands back `sanitizeDocHTML(editor.getHTML())`, old execCommand HTML is normalised on load (`legacyToEditorHTML`). Reused for Self Notes and, via `bindNoteEditor`, every note box. Mounted editors are tracked in `liveEditors` and destroyed once their element leaves the DOM. Indentation (`psmEditorExtensions`): the bullet/ordered list nodes may contain lists directly, so `indent()` can nest the first item of a list or stack levels without a sibling (wrap + join), `outdent()` lifts directly-nested items via `liftTarget`, and paragraphs/headings carry an `indent` attribute rendered as `margin-left` (kept by `sanitizeDocHTML`); the toolbar, Tab/Shift+Tab, Ctrl+]/[ and Ctrl+M call the same two commands, and a high-priority keymap handles Enter/Backspace on directly nested items. Numbering: a ProseMirror plugin (`psmListNumbering`, appendTransaction + an on-create pass flagged `preventUpdate`) sets `start` on every ordered list that follows another one with only blank paragraphs in between, and resets it to 1 otherwise — `start` is the rule's output, never user input; the sanitizer keeps it and `htmlToText` reads it.
