...
8. **Half-screen windows cut off / scrolled the tab row (Sep 2026)**: below 900px the sidebar becomes a horizontal tab row that was one non-wrapping line with a sideways scroll, so the last tabs were hidden. Guard: the row **wraps** ( in the 900px block). **Check protocol:** at 960 / 820 / 700 / 560 / 430px, every tab in BOTH platforms must render with no element past the viewport edge and  (the preview probe in the session log does exactly this).
