...
| 1.12 | **Cloud snapshot history**: full-workspace copies every 4h of activity — the SERVER gates automatic ones to one per 4h across all devices and skips unchanged states (`snapshots:add`, `force` for the manual "⛅ Snapshot to cloud now" button); metadata rows (`snapshots`) are separate from the blobs (`snapshotBlobs`) so listing/pruning never reads the copies; keep 30, prune oldest; never snapshots an untouched seed; **fails soft** if the cloud rejects it | S15 + signed-in 8.3 |
...
| 1.21 | **Push batching (bandwidth)**: every keystroke still saves locally at once, but the cloud gets **one push per pause in typing** (`schedulePush`: idle 1.5 s, max wait 8 s), **single-flight** (a save during an in-flight push is sent by the ack), and **flushed immediately** when the tab is hidden, the page closes, the connection returns, or a restore runs (`flushPush`). The sync pill says "Saving…" from the first keystroke and "Synced" only after the ack, as before | S22, S14 |
| 1.22 | **Version subscription (bandwidth)**: devices subscribe to the tiny `workspace:version` record (`{updatedAt, writerDeviceId}` in `workspaceMeta`), not the 80 KB blob. The blob is fetched once on connect (`workspace:get`) and then only when ANOTHER device changed it; a device's own write comes back as a version tick that just confirms the ack. `workspace:save` decides on the meta row and patches the blob by id (no 80 KB read), and an unchanged re-push writes nothing. Out-of-order fetches are ignored (`verSeen`). Before this, one keystroke cost ~(2 + devices) × 83 KB of database bandwidth (~500 MB/day) | S21, S22, S16 |
...
| # | Behavior |
|---|----------|
| 2.1 | Sign-in gate: **email + 6-digit code** (Convex action `auth:requestCode` → Resend email → `auth:verifyCode` → session token in localStorage `psmSession_v1`), locked to the single `ADMIN_EMAIL` **server-side**; codes hashed, 10-min TTL, **5 wrong tries (the counter commits — `consumeCode` returns a failure instead of throwing, since a throw rolls a mutation's writes back)**, used/dead codes are marked consumed rather than deleted so the 30s cooldown and 15/day cap keep counting; sessions ~6 months, max 10, **stored as SHA-256 hashes** (pre-Sep-2026 raw tokens are upgraded on first use). The email field is prefilled; "← different email / resend" goes back. A remembered device boots straight in; if the session is gone the server answers `null` and the gate reappears with "Your sign-in expired". Offline with a remembered session: after 6s the local copy opens and syncs when back online. **[ALWAYS CHECK]** |
...
| 2.7 | **Security log**: append-only, newest 30 (sign-ins, device events, `data_restored`, `data_shrunk`, `stale_overwrite_blocked`) with friendly labels. Server-side: event names validated, detail capped at 1000 chars, an unapproved device may only log `sign_in` / `device_requested`, rows older than 90 days pruned by the daily cron (`crons.ts` → `maintenance:cleanup`, which also drops stale codes and expired sessions) |
...
| 3.2 | Topbar: view title, week pill (`W##`), **sync pill** (see 1.13), **💼 Work ⇄ 🌞 Personal switch** (restyles the whole app via `data-mode`), theme toggle (sun/moon, persists in settings), shield button, account button (avatar + email) |
...
### 4.2 To-Do
- **Two columns — Mandatory | Optional**: every task carries an `optional` flag (default **false** = Mandatory, so existing tasks all start Mandatory). Each column has its own header with a live count and its own time sections; the columns stack on narrow screens. **Relabel** a task three ways: the **Mandatory/Optional chip** on every task row (To-Do, project page, meeting page), **drag** the row's ⋮⋮ grip into the other column, or pick the kind in the **quick-add bar** before logging. Optional rows get a dashed left border. Overdue/completed **projects** always sit in the Mandatory column (they're commitments). The flag is normalized (`normTask`), synced with the blob, and covered in S12.
- **Quick-add bar**: text (Enter adds), project selector (or Ad-hoc), **Mandatory/Optional selector**, deadline picker (calendar + quick options incl. `+3 business days`, week-type deadlines, and **No deadline**). Default is today; picking **No deadline** logs the task without a date — it still shows in To-Do under **No date** (just not on the Calendar/Year). Tasks are no longer forced to have a deadline anywhere: the picker's full-width **No deadline** preset replaced the old "Clear (no deadline)" button and works for tasks, projects, the create modal and the task modal alike.
- Sections inside each column: **⚠ Overdue** (tasks AND overdue projects — project rows show 🚩 name + `Project` chip + **category chip** + reschedulable deadline chip + complete-check), **Today**, **This week**, **Upcoming**, **No date**, **Completed** (toggle; includes completed projects with restore). An empty column shows a drop hint.
- Task rows everywhere: done check, inline text edit, project tag (click = jump to project/meeting), Mandatory/Optional chip, deadline chip picker (`· overdue` state), 🗒 notes post-it, delete. Search filters tasks + notes + project names.
...
- **The meetings list is collapsible**: ⇤ Hide list / ☰ Show list in the toolbar (remembered per device in localStorage `psmUI_mtgList`); while hidden, a dropdown in the toolbar switches meetings and the meeting page takes the full width.
...
- **Every note box is a compact rich editor** (meeting-log agenda, project-page Notes, project meeting-note sections, Week Prep notes) — the same Tiptap (ProseMirror) editor as Documents (`bindRichEditor`, §4.7) in a mini form (`noteEditorHTML`/`bindNoteEditor`): **real bullet and numbered lists with proper hanging indents and nesting** (Tab / Shift+Tab, nested markers ○ ▪ / a. i.), `- ` / `* ` / `1. ` / `1) ` autoformat while typing, Ctrl+Shift+8 / 7 toggles, Enter on an empty item ends the list, smart paste of pasted lists (plain text or Outlook/Word HTML), and a small toolbar (lists, outdent/indent, bold, italic, highlight, clear) that brightens on hover/focus and lights up the active formatting. Stored as **sanitized HTML** with a per-object `rich` flag (`en.rich`, `note.rich`, `p.notesRich`, `weekPrep.rich`); older plain text (including the former "  • " / "  1) " smart-list lines) is converted on display (`plainToHTML`: markers → `<ul>/<ol>`, indent depth → nesting) and becomes HTML on the first edit — a note's other sections convert in that same step so a half-rich object can never exist. Every reading path (collapsed-card snippets, log search, ⧉ copy) goes through `noteText()` (`htmlToText`, which also folds Tiptap's `<li><p>…</p></li>` items and `<br>` soft breaks). The converters are pure and covered by S20. **[ALWAYS CHECK]** in a meeting agenda: type `- item`, Enter, Tab → a nested bullet with a real hanging indent; numbered items keep their numbers aligned. (The plain-text smart-list textareas, the CSS hanging-indent workaround and the hand-rolled contenteditable/execCommand editor were all replaced in Sep 2026 — none of them could keep list structure stable; Tiptap owns the document model now.)
...
### 4.6 Week Prep — per-week preparation: notes (rich note editor — see §4.10) + check-off item list, week navigation.

### 4.7 Documents — rich-text documents (title + formatted body), create/edit/delete.
- **Shared rich editor** (`bindRichEditor`, also used by Self Notes and — via `bindNoteEditor` — every note box) **[ALWAYS CHECK]**. Since Sep 2026 it is a real editor engine, **Tiptap (ProseMirror)**, vendored as `vendor/tiptap.js` (built from `tools/editor-entry.js` by `npm run build:editor`; no CDN, works offline; its base CSS sits in the stylesheet because the CSP forbids injected styles — `injectCSS:false`). Word-style **autoformat while typing** — `- ` / `* ` / `+ ` → bullet list, `1. ` → numbered list, `# ` / `## ` / `### ` → headers, `> ` → quote (all at the start of a line; Ctrl+Z undoes the conversion). **Tab / Shift+Tab** nest / un-nest list items (markers • ○ ▪ / 1. a. i.) — **at any point and any depth**: the first item of a list can be pushed deeper and levels stack freely (lists may nest directly, `psmEditorExtensions`; ProseMirror's stock command only nests under a previous sibling), and the **⇤ / ⇥ toolbar buttons**, Ctrl+] / Ctrl+[ and Ctrl+M / Ctrl+Shift+M do exactly the same; outside a list the same buttons/keys **indent / outdent the paragraph itself** (`margin-left`, up to 8 steps) **[ALWAYS CHECK — click the indent button on the first bullet of a list, twice, then outdent back]**. Enter continues a list, Enter on an empty item ends it (an empty nested item steps one level up), Backspace at the start of an item or an indented paragraph steps one level out, **numbered lists separated only by blank lines continue their numbering** (`1. a` / blank / `1. b` shows 1. and 2.; the follow-on list carries `start="2"`, recomputed as the first list changes and kept by the sanitizer; any text or bullet list in between restarts at 1) **[ALWAYS CHECK]**, **Enter after a header returns to normal text**, **Ctrl+B / I / U**, **Ctrl+Shift+8 / 7** toggle bullet / numbered lists, **Ctrl+Shift+H** highlights, Ctrl+Z / Y undo / redo. **Smart paste**: pasted plain text whose lines look like a list becomes a real nested list (indent = level, `plainToHTML(…,{zeroBase:true})`), **Outlook / Word HTML lists** (`MsoListParagraph` lines) are rebuilt as real lists (`msoListsToHTML`), any other HTML is reduced to the schema (never messy markup). Toolbar: Header/Sub/Text, B/I/U/S, highlight, bullet + numbered lists, **Quote**, indent/outdent, align, undo/redo, Clear — buttons light up for the formatting under the caret. Storage: the editor's HTML through `sanitizeDocHTML` on every change (`<ul><li><p>…</p></li></ul>`, `<strong>`, `<em>`, `<mark style="background-color">`, `<p style="text-align">`); HTML written by the old execCommand editor (`<div>` lines, `<div><br></div>` blank lines, `<span style="background-color">` highlights, `<b>`/`<i>`) loads unchanged (`legacyToEditorHTML`) and is rewritten in the new shape on the first edit. Word counts / search use `docText`.
...
7. **Every second keystroke reverted (Aug 30 2026, the barrier's first day live) [ALWAYS CHECK — type a few quick letters in a document while signed in]**: Convex echoes each write back through the subscription BEFORE resolving the mutation ack, so with two keystrokes inside one round-trip the echo of letter 1 arrived while the device held letter 2 and nothing was marked "seen" yet → the barrier took our own write for an unseen foreign version, reverted letter 2, and toasted "this device was behind". Guard: each device persists the stamps it pushed itself (`psmData_v1_own`, `noteOwn`/`isOwn`) and the barrier ignores an echo of its own write (matched by equality, never by ordering, so a stale device's own fresh stamps still can't "see" anything). Also fixed alongside: a **rejected** push no longer marks the cloud's unseen version as seen (it would have let the adoption path skip its restore point), and an echo mid-flight no longer triggers a duplicate push. S21 reproduces the race (fails without the fix).
8. **Half-screen windows cut off / scrolled the tab row (Sep 2026)**: below 900px the sidebar becomes a horizontal tab row that was one non-wrapping line with a sideways scroll, so the last tabs were hidden. Guard: the row **wraps** (`.nav{flex-wrap:wrap}` in the 900px block). **Check protocol:** at 960 / 820 / 700 / 560 / 430px, every tab in BOTH platforms must render with no element past the viewport edge and `documentElement.scrollWidth <= innerWidth` (the preview probe in the session log does exactly this).
9. **Sign-in attempt counter never persisted (found by the Convex reviewer, Sep 2026)**: `consumeCode` incremented `attempts` and then threw; a throw rolls back a Convex mutation's writes, so the "5 tries" cap did not exist. Guard: the mutation returns `{ok:false,error}` (throw happens in the action afterwards); S16 statically asserts `consumeCode` contains no `throw`. **[ALWAYS CHECK] after touching auth.ts.**
10. **500 MB/day of database bandwidth (Sep 2026)**: whole-blob push per keystroke × (mutation read + write + one subscription re-read per device). Guard: push batching (1.21) + version subscription (1.22); the snapshot split (1.12) removed the other big reader. **Check protocol:** after a day of use the Convex dashboard's database bandwidth should be single-digit MB.
