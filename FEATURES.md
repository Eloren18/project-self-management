...
- **Every note box is a compact rich editor** (meeting-log agenda, project-page Notes, project meeting-note sections, Week Prep notes) — the same Word/Outlook-style editor as Documents (`bindRichEditor`) in a mini form (`noteEditorHTML`/`bindNoteEditor`): **real bullet and numbered lists with proper hanging indents and nesting** (Tab / Shift+Tab, nested markers ○ ▪ / a. i.), `- ` / `* ` / `1. ` / `1) ` autoformat while typing, Ctrl+Shift+8 / 7 toggles, Enter on an empty item ends the list, smart paste of pasted lists, and a small toolbar (lists, outdent/indent, bold, italic, highlight, clear) that brightens on hover/focus. Stored as **sanitized HTML** with a per-object `rich` flag (`en.rich`, `note.rich`, `p.notesRich`, `weekPrep.rich`); older plain text (including the former "  • " / "  1) " smart-list lines) is converted on display (`plainToHTML`: markers → `<ul>/<ol>`, indent depth → nesting) and becomes HTML on the first edit — a note's other sections convert in that same step so a half-rich object can never exist. Every reading path (collapsed-card snippets, log search, ⧉ copy, Excel export) goes through `noteText()` (`htmlToText`). The converters are pure and covered by S20. **[ALWAYS CHECK]** in a meeting agenda: type `- item`, Enter, Tab → a nested bullet with a real hanging indent; numbered items keep their numbers aligned. (The plain-text smart-list textareas and the CSS hanging-indent workaround were removed in Sep 2026 — they could never align wrapped lines per item.)
